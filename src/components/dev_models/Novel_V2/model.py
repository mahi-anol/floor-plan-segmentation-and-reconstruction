...
from src.components.dev_models.Novel_V2.inverted_acb import InvertedACB
from src.components.dev_models.Novel_V2.attention_block import CoordinateAttention 
...
class MULTI_UNIT_FLOOR_SEGMENT_MODEL(nn.Module):
    def __init__(self, image_channel, class_num):
        ...
        self.conv12 = nn.Sequential(nn.MaxPool2d(2, 2), InvertedACB(c[0], c[1], stride=1))
        self.conv2 = nn.Sequential(nn.MaxPool2d(2, 2), InvertedACB(c[0], c[1], stride=1), InvertedACB(c[1], c[1], stride=1))
        ...
        self.conv3  = nn.Sequential(nn.MaxPool2d(2, 2), InvertedACB(c[1], c[2]), InvertedACB(c[2], c[2]))
        ...
        self.conv4  = nn.Sequential(nn.MaxPool2d(2, 2), InvertedACB(c[2], c[3]), InvertedACB(c[3], c[3]))

        # Stage 5 (Bottleneck)
        self.conv5 = nn.Sequential(nn.MaxPool2d(2, 2), InvertedACB(c[3], c[4]), InvertedACB(c[4], c[4]))
        ...
        # --- HEADS ---
        # 1. Main Semantic Head
        self.final = nn.Conv2d(c[0], self.class_num, 1)
        
        # 2. NEW: Edge Segmentation Head
        # Branches off from high-res feature 'out1' (c[0] channels)
        # We use a dedicated convolution block to specialize features for edges
        self.edge_head_conv = InvertedACB(c[0], c[0] // 2)
        self.edge_final = nn.Conv2d(c[0] // 2, 1, 1) # Output 1 channel (Binary Edge)
    ...
    def forward(self, x):
        ...
        seg_out = self.final(out1)
        
        # Edge Segmentation Output
        edge_feat = self.edge_head_conv(out1)
        edge_out = self.edge_final(edge_feat)
