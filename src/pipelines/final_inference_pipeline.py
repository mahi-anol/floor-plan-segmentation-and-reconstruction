...
def generate_3d_scene(polygons, output_filename="output_floorplan.obj", extrusion_heights=None):
    ...
    Includes logic to handle MultiPolygon errors from self-intersecting JPG artifacts.
    """
    ...
    for poly in polygons:
        ...
        try:
            ...
            print(f"[WARN] Skipping invalid polygon creation: {e}")
            continue
        
        # 3. Validation & Repair
        if not shapely_poly.is_valid:
            # buffer(0) attempts self-repair but may split the polygon into multiple pieces
            shapely_poly = shapely_poly.buffer(0) 

        if shapely_poly.is_empty:
            continue

        # 4. Handle Single Polygon vs MultiPolygon
        # If buffer(0) returned a MultiPolygon, we need a list of sub-polygons.
        # If it's a regular Polygon, we wrap it in a list.
        if shapely_poly.geom_type == 'MultiPolygon':
            poly_list = list(shapely_poly.geoms)
        else:
            poly_list = [shapely_poly]

        # 5. Determine Height
        height = extrusion_heights.get(cls_id, 10.0) 

        # 6. Extrude all parts
        for sub_poly in poly_list:
            try:
                # Trimesh extrusion
                mesh = trimesh.creation.extrude_polygon(sub_poly, height)
                
                # Styling: Assign a random color
                mesh.visual.face_colors = trimesh.visual.random_color()
                
                meshes.append(mesh)
            except Exception as e:
                # Catch specific extrusion errors (e.g. if the polygon became too small)
                print(f"[WARN] Failed to extrude polygon fragment: {e}")

    if not meshes:
        print("[WARNING] No meshes created. Check polygon extraction.")
        return

    # 7. Combine all meshes into one scene
...
def run_pipeline(config):
    ...
    # Example Override: If you know ID 1 is Wall, make it taller
    HEIGHT_MAP[1] = 50.0 
...
if __name__ == "__main__":
    ...
    CONFIG = {
        ...
        "model": r"E:\floor-plan-segmentation-and-reconstruction\checkpoints\cross_entropy_best.pt",
